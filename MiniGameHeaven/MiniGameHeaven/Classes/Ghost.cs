    ...
    public class Ghost
    {
        ...
        public void Set_Ghosts()
        {
            ...
            for (int y = 0; y < 30; y++)
            {
                for (int x = 0; x < 27; x++)
                {
                    if (pac_man.gameboard.Matrix[y, x] == 15)
                    {
                        Amount++;
                        xStart[Amount] = x;
                        yStart[Amount] = y;
        ...
        private bool direction_ok(int x, int y, int ghost)
        {
            ...
            if (pac_man.gameboard.Matrix[y, x] < 4 || pac_man.gameboard.Matrix[y, x] > 10) { return true; } else { return false; }
        }
        ...
        private void Other_Direction(int direction, int ghost)
        {
            ...
            if (pac_man.gameboard.Matrix[yCoordinate[ghost], xCoordinate[ghost]] < 4)
            {
        ...
        public void CheckForPacman()
        {
            ...
            for (int x = 0; x < GhostAmount; x++)
            {
                if (xCoordinate[x] == pac_man.pacman.xCoordinate && yCoordinate[x] == pac_man.pacman.yCoordinate)
                {
                    switch (State[x])
                    {
                        case 0: pac_man.player.LoseLife(); break;
                        case 1:
                            State[x] = 2;
                            hometimer.Enabled = true;
                            GhostImage[x].Image = Properties.Resources.eyes;
                            pac_man.player.UpdateScore(300);
                            break;
                    }
                }
            }
